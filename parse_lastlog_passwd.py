...
def load_users_from_passwd(passwd_path):
    users = []
...
def read_lastlog(lastlog_path, passwd_path, csv_output_path="lastlog_output.csv"):
    users = load_users_from_passwd(passwd_path)
    ...
    with open(lastlog_path, "rb") as f, open(csv_output_path, mode="w", newline="") as csvfile:
        ...
        for uid, username in users:
            ...
            try:
                ...
                writer.writerow([username, line, host, last_login])
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python parse_lastlog.py /path/to/lastlog /path/to/passwd [output.csv]")
        sys.exit(1)

    lastlog_path = sys.argv[1]
    passwd_path = sys.argv[2]
    output_csv = sys.argv[3] if len(sys.argv) > 3 else "lastlog_output.csv"
    ...
    if not os.path.exists(passwd_path):
        print(f"Error: File not found: {passwd_path}")
        sys.exit(1)

    read_lastlog(lastlog_path, passwd_path, output_csv)
